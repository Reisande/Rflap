...
function CFG_Visual() {
  ...
  const definition_plus_handler = (button_press) => {
    ...
    !bool_first_mount
      ...
      : (bool_first_mount = bool_first_mount);
    bool_first_mount = true;
  ...
  useEffect(() => {
    ...
    document.addEventListener("input", (e) => {
      ...
      if (
        e.target.id == "rule_terminal" ||
        e.target.id == "rule_non-terminal"
      ) {
  ...
  const exportJson = (e) => {
    ...
    if (readImportTxt != null) {
      ...
      if (
        inputValForExport.length > 7 &&
        inputValForExport.includes("@uic.edu")
      ) {
        ...
        let i = def.length;
        if (def == null) {
          Object.keys(CFGJsonImport.userInputCFG.productions).forEach((arr) =>
            definition_plus_handler({})
          );
        } else {
          while (
            i < Object.keys(CFGJsonImport.userInputCFG.productions).length
          ) {
            definition_plus_handler({});
            i += 1;
          }
      ...
      if (
        ...
        downloadObjectAsJson(exportToJson, "RFLAP_" + "CFG", inputValForExport);
        setExportModal(false);
        setDisplayWarning(false);
      } else {
        setDisplayWarning(true);
  ...
  const preprocessor = () => {
    ...
    master_context.grammar_obj.forEach((rule_obj, id) => {
      ...
      if (rule_obj.NON_TERM != rule_obj.NON_TERM.toUpperCase()) {
        alert(
          "terminal (lower-case letter or numeric) in left-hand non-terminal position!"
        );
        error_found = true;
        return;
      }
      ...
      if (
        packet.productions[NON_TERM] == undefined ||
        packet.productions[NON_TERM] == null
      )
        packet.productions[NON_TERM] = [];
      ...
      packet.productions[NON_TERM] =
        packet.productions[NON_TERM].concat(arr_of_arr_of_chars);
    });
  ...
  function on_click_CFG_api(e) {
    ...
    if (
      array_of_rules == null ||
      array_of_rules == undefined ||
      array_of_rules.length == 0
    ) {
      alert("Make rules in the left definition table!");
      return;
    } else {
      grammar_with_funcs = Grammar(array_of_rules);
    }
    user_input_row_collection = [
      ...Array(HTMLCol_to_array(row_ref_container.current.children).length),
    ];
    ...
    input_strings_CFG.forEach((test_string) => {
      bool_results.push(
        parser.parse(grammar_with_funcs, test_string).length > 0 ? 2 : 0
      ); // true
    });
