...
import accept_bar from '../images/accept.svg';
import add_bar from '../images/add-bar.svg';
import points_bar from '../images/points.svg';
import reject_bar from '../images/reject.svg';
import transition_bar from '../images/transition.svg';
import blank_svg_bar from '../images/blank.svg';
import passive_bar from '../images/delete.svg';
import remove_bar from '../images/remove.svg';
//Yo
...
function Visual() {
  ...
  const wrapper: any = useRef(null); //Display graph in div "wrapper"
  // const [img_array_index,set_img_array_index] = useState(0);//change to enums
  const img_status: any = useRef(null);
  ...
  useEffect(() => {
    ...
    network.on('hoverNode', params => {
      ...
      if (in_add_node_mode) {
        ...
        network.moveNode(new_id, (Math.random() - 0.6) * 200, (Math.random() - 0.7) * 200);
  ...
  function mount_styling() {
    ...
    let current_time;
    fetch(url, postingObject).then((callback: any): any => {
  ...
  function populateNode(props) {
    ...
    network.moveNode(node_id_global, (Math.random() - 0.6) * 400, (Math.random() - 0.6) * 400);
  }
  ...
  return (
    <div id="non-header-div">
      ...
      <div className="div-inline-group-below-header">
        <div id="trash_button" className="div-inline-group-below-header">
          <input
        ...
        <div id="add_button_visual" className="div-inline-group-below-header">
          <input
            id="add_button_image"
        ...
        <ButtonGroup id="group-holder" className="mr-2">
          <Button className="visual-button" variant="info" onClick={toEditEdgeMode}>
            {' '}
            <span style={{ color: 'white' }}>Add Transitions</span>
          </Button>
          {/* <Button className="visual-button" variant="secondary" onClick={toAddNodeMode}><font color='yellow'> Add Node</font> </Button> */}
          <Button className="visual-button" variant="info" onClick={setInitial}>
            {' '}
            <span style={{ color: 'white' }}>Mark Initial</span>
          </Button>
          <Button className="visual-button" variant="info" onClick={setAccepting}>
            {' '}
            <span style={{ color: 'white' }}>Mark Accepting </span>
          </Button>
        </ButtonGroup>
