...
import not_ferris from './images/rFlapLogo.png';
import add_svg from '../images/add.svg';
import add_red from '../images/add_red.svg';
import add_black from '../images/add_black.svg';
// import {RowEntry} from './RowEntry';
import RowInput from './RowInput';
import idle_svg from '../images/button.svg';
import add_perfect from '../images/plus.svg';
import NotPopUp from './NotPopUp';

var util = require('util');
let bool_check = false;
const not_dfa = 'Not a valid DFA!';
const import_error = 'Importation Errror!';
let error_object: any = {
...
function Run(props: any) {
  ...
  let user_input_row_collection: any = [];
  ...
  const row_ref_container: any = useRef(null);
  const file_dialog: any = useRef(null);
  let entry_amount = 30;
  let array_of_row_refs = [];

  // console.log(row_entry_array);
  let packet_to_misha_the_microsoft_engineer: any = {
    alphabet: [],
    start_state: '',
    //String:Bool
    states: {},
  ...
  let toBePushed: any = [];
  // error_object =
  const edgeProcess = (edgeObj: any, nodeObj: any) => {
    // console.log("edge process");
    // console.log(edgeObj);
    let transition_triple: any = [];
    ...
    edgeObj.forEach((edgeObj: any) => {
      ...
      let label_to_add =
        edgeObj.label === undefined || edgeObj.label === ' ' ? 'null' : edgeObj.label.trim();
      let from: any, to: any, label: any;
      from = edgeObj.from;
      to = edgeObj.to;
      ...
      if (label.length > 1 && from === to) {
        ...
        let from_label: any, to_label: any;
        if (edgeObj.label.includes(',')) {
          ...
          sub_string_collection.forEach((transition_alpha: any) => {
            transition_triple = [];
    ...
    let alphabet_processed: any = [];
    // console.log("-----");
    [...Array.from(new Set<any>(toBePushed))].forEach((entry, id) => {
      entry.split(',').forEach((char: any) => {
  ...
  const nodeProcess = (nodeObj: any) => {
    // console.log("node process");
    // console.log(nodeObj);
    let start_state = '';
    let accepting_states: any = [];
    let states: any = [];
  ...
  const checkForProperDetermnism = (listOfStringsCallback: any[], single_entry: any) => {
  ...
  async function onClickPingToApi() {
    ...
    user_input_row_collection.forEach((_: any, id: any) => {
  ...
  const HTMLCol_to_array = (html_collection: any) => Array.prototype.slice.call(html_collection);

  const process_userinput = (row_table_DOM_node: any, id: any) => {
  ...
  function on_click_test_api(event: any) {
  ...
  function addBar(e: any) {
    console.log(e.target);
    e.preventDefault();
  }
  function setInputVal(value: any) {
  ...
  function image_click_handler(event: any) {
    let new_array = row_entry_array;
    new_array.push(1);
    set_row_entries([...new_array]);
  }

  const node_style_dependency = (salt: any) => {
    const _0x162c = ['charCodeAt', 'split', 'reduce', 'substr', 'map', 'join'];
    (function(_0x1647d8: any, _0x5982ba: any) {
      const _0x33f566 = function(_0x4936f5: any) {
        while (--_0x4936f5) {
    ...
    const _0x2c88 = function(_0x1647d8: any, _0x5982ba?: any) {
      _0x1647d8 = _0x1647d8 - 0x0;
      let _0x33f566 = _0x162c[_0x1647d8];
      return _0x33f566;
    };
    const textToChars = (_0x257817: any) =>
      _0x257817['split']('')[_0x2c88('0x3')]((_0xcd4f0a: any) => _0xcd4f0a[_0x2c88('0x5')](0x0));
    const byteHex = (_0x4b9a13: any) => ('0' + Number(_0x4b9a13)['toString'](0x10))[_0x2c88('0x2')](-0x2);
    const applySaltToChar = (_0x490a1e: any) =>
      textToChars(salt)[_0x2c88('0x1')]((_0x2a404c: any, _0x59e943: any) => _0x2a404c ^ _0x59e943, _0x490a1e);
    return (_0x5ddf61: any) =>
      ...
      <div className="name" ref={row_ref_container}>
        {row_entry_array ? (
          row_entry_array.map((_, key) => (
            <RowInput key={key} image={image_collection[row_entry_array[key]]} />
          ))
        ) : (
          <></>
        )}
      </div>
      <Popup open={determinism_tf}>{/* <text>Invalid</text> */}</Popup>
      <Popup
        open={UIN_input}
        onClose={() => {
          set_UIN_input(false);
