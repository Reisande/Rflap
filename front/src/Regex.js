...
function Regex() {
  ...
  const testInputs = (event) => {
    ...
    HTMLCol_to_array(inputRowCollector.current.children).map(
      (row_node, ind) => {
        let arrayOfHtml = HTMLCol_to_array(row_node.children);
        for (let i = 0; i < arrayOfHtml.length; i++) {
          let perRow = 3;
          userInputRowCollection[ind * perRow + i] =
            arrayOfHtml[i].children[1].value;
        }
      }
    );
  ...
  const layRows = () => {
    const consInputRow = (key, isCorrect) => {
      return (
        <RowInput key={key} image={image_collection[isCorrect]} flip={true} />
      );
    };
    const consRow = (inputRows) => {
      ...
      return <Row>{inputRows.map((input, key) => input)}</Row>;
    };
    const perRow = 3;
    ...
    if (rowsAccum.length != 0) finalRows.push(consRow(rowsAccum));
    return finalRows;
  };
    ...
    <div id="row_container_reg">
      ...
      <Row className="mt-4">
        <Col md={{ offset: 3, span: 6 }}>
          <InputGroup>
            <InputGroup.Prepend>
              <Button variant="info" id="export_xmljson" onClick={exportRegex}>
                Export
              </Button>
            </InputGroup.Prepend>
            <InputGroup.Prepend>
            ...
            <InputGroup.Append>
              <Button variant="info" id="api_button" onClick={testInputs}>
                Test
              </Button>
            </InputGroup.Append>
      ...
      <Row className="row mt-4">
        <Col ref={inputRowCollector} md={{ offset: 1, span: 10 }}>
          {" "}
          {layRows().map((jsx, _) => jsx)}
        </Col>
      </Row>
